-- | Generate @.proto@ files from servant-reimagined API types.
--
-- @
...
-- @
module Servant.Reimagined.Grpc.Generate
  ( -- * Proto generation
    generateProto
  , generateProtoFull
    -- * Message definitions
...
import Servant.Reimagined.Grpc.Proto (ProtoMessageName(..), ProtoField(..), ProtoFieldType(..))
